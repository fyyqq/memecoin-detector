...
> is INCLUSIVE, the ceiling is EXCLUSIVE (a peak of exactly $1B does NOT
...
3. **Qualify for the main list when age ≤ 30 days AND a VERIFIED or OBSERVED
   ...
   **stays qualified**. The ceiling is also on the peak — a token whose
   verified/observed MC **ever reached or exceeded $1B is excluded even if its current MC
...
- **Historical qualification engine (Step 13C, Strategy D)** runs in the pipeline
  ...
  qualified; a peak that ever reached or exceeded $1B is excluded and is **not** re-qualified
...
- **Read API `GET /api/memecoins`** — read-only, PostgreSQL only, never calls
  ...
  cap peak **in `[$5M, $1B)`** (`GREATEST(observed_peak, historical_peak_value)` < $1B — a peak at or above $1B is excluded even when current MC is back in
...
- **Detail API `GET /api/memecoins/{chainId}/{tokenAddress}`** — read-only,
  ...
  **`risk_assessment`** (`status` `pending`/`completed`/`partial`/`failed`,
  ...
  peak cleared $5M but reached or exceeded $1B, else `null`),
